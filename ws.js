...
const messageObservable$ = connectSubject$.pipe(mergeMap(ws =>
    new Observable(subscriber => {
        ws.on('message', (data) => subscriber.next({
...
const ofType = (messageType) => {
    return (observable$) => observable$.pipe(filter(({ message: { eventType } } = {}) => eventType === messageType));
}

const subscribeToResponse = (controllerObservableCollection) => {
    return from(controllerObservableCollection).pipe(mergeAll()).subscribe(({ ws, response }) => {
